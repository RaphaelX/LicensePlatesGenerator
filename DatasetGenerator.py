...
for plate in plates:
    ...
    while count < ratio:
        ...
        label = find_labels_for_yolo(scene.shape, ntlc, nblc, ntrc, nbrc, new_center,category_number="0")
        ...
        cv2.imwrite(yolo_path+"/"+str(count_yolo)+".jpg", final_image)
        txt_outfile = open(database_path+"/"+str(count_yolo)+".txt", "w")
        txt_outfile.write(label+"\n")
        txt_outfile.close()
        ...
        # cv2.imshow('allgray_image',allgray_image)
        # print(plate)
        # print(scene_path)
        # print("lp1: "+str(lp1)+" lp2: "+str(lp2)+" sc1: "+str(sc1)+" sc2: "+str(sc2))
        # print(sc1)
        # print(nbrc[0,0])
        # cv2.waitKey(100)